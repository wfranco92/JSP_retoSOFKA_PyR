...
<html>
    <head>
        ...
        <title>Preguntas y Respuestas. 5 Ronda</title>
    </head>
    <%
        
        // el modelamiento y logica de esta ronda es igual a la anterior.
        int numPregunta=PreguntaAleatoria.generarnumero(5);
        ...
        Ronda ronda = new Ronda(1, pregunta.getEnunciado(), listaRespuestas, 40);
    
    %>
    <body>
        <div class="container mt-5">
        <script>alert("respuesta correcta!!!. Puntaje obtenido: 30 puntos")</script>
        ...
        <div class="container mt-5">
            <h4>Puntaje Actual: <%=ronda.getPuntos()+20 %> Puntos</h4>
            <h4>Jugador: <%= sesion.getAttribute("nombre") %>  </h4>
        </div>
        <div class="container mt-5 d-grid gap-2 d-md-block">
            <a href="abandonarPartida.jsp?puntos=<%=ronda.getPuntos()+20 %>&ronda=<%=ronda.getNumRonda()%>" class="btn btn-primary">Terminar partida con puntaje acumulado</a>
        </div>
            <%
